...
3. シンプルなファイル名でコピーを作成する:
   - 画像: `{YYYY-MM-DD}.png`（生成されたPNGをコピー）
   - キャプション: `{YYYY-MM-DD}.md`（instagram-latest.json の caption を Write で書き出す）
4. Surface にSCPで転送する:
   - `scp <ファイル> stsrj@surface:"/mnt/c/Users/stsrj/Desktop/Instagram投稿/"`
5. 生成結果を画面に表示する（「画像生成 画面表示フォーマット」参照）
