...
arguments:
  - name: memo
    description: "塾長の一言メモ（例: 小6体験生が来た。算数の穴埋めて中1まで進んだ。笑顔で帰った）"
    required: true
---
...
1. **学習タイムライン**: 生徒詳細ページのデフォルト表示。`take_screenshot` で撮影。
2. **マイレベル**: 「マイレベル」タブに切り替えて `take_screenshot` で撮影。
   - `take_snapshot` → 「マイレベル」の uid で `click(uid)`

#### 1e. 名前マスク

スクショ内の生徒名を隠す。`evaluate_script` で名前要素を白い矩形で覆ってからスクショを撮り直す:

```javascript
() => {
  const allText = document.querySelectorAll('*');
  for (const el of allText) {
    if (el.children.length === 0 && el.textContent.includes('${STUDENT_NAME}')) {
...
STEP 1 の 1a〜1f をそのまま実行する。生徒名は引数から取得。
