...
3. **名前の読み方（フルネーム）**: 漢字の名前から読み方の候補を複数生成し、**選択肢形式**で提示する。ユーザーは番号を選ぶだけでよい
   - 姓と名それぞれの一般的な読み方を組み合わせて**フルネームの候補**を出す
   - 読みが一意に定まる姓名（例: 鈴木→すずき）は自動解決し、曖昧な部分だけ組み合わせる
...
>   2. 単元は？
>   3. お名前の読み方は？
>      a) すずき あいり  b) すずき えり  c) その他（教えてください）」
