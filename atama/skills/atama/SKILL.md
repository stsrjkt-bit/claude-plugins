...
| `/atama <生徒名> <単元名を含む指示>` | **指定単元の補習プリント作成（つまずきマーク不要）** |
| `/atama login` | Chromium起動＆ログインのみ（Phase 1〜2） |

## 単元指定モード（つまずきマークなしでも対応）

ユーザーが特定の単元名を指示した場合（例:「回転体の表面積と体積のプリント作って」）、**つまずきマークの有無に関係なく** その単元の補習プリントを作成する。

**フロー:**
1. Phase 1-3: Chromium起動 → ログイン → 生徒詳細遷移（通常通り）
2. Phase 4 変更: つまずきスキャンの代わりに、指定単元を「単元ごとの学習状況」で検索して学習データを取得
3. Phase 5 スキップ: 単元は既にユーザーが指定済み（名前の読み方の確認のみ行う）
4. Phase 6-8: 指定単元の誤答分析（通常通り）
5. Phase 9以降: 補習プリント＆動画作成（通常通り）

**判定基準:**
- ユーザーの指示に具体的な単元名（「回転体」「おうぎ形」「連立方程式」等）が含まれていれば単元指定モード
- 「最新の」「最近の」等の曖昧指定 → つまずきスキャンモード（Phase 4-5 を通常実行）
- 教科名だけ（「中学数学」）→ つまずきスキャンモード
...
| **Codex Exec 1** | **Phase 2-4, 6** | **Codex** | ログイン→生徒遷移→つまずきスキャン→誤答分析を一気通貫 |
| Phase 5 | つまずき報告・選択 | Claude | ユーザーとの対話（Codex は対話不可） |
| Phase 7 | 結果まとめ | Claude | Codex の出力を分析・整形して報告 |
...
Phase 6: 誤答分析
[Phase 6 の手順。Phase 5 は Codex 側ではスキップ — 全つまずき単元を対象にする]

成功条件:
- 全教科のつまずき単元一覧が取得できた
- つまずき単元のモーダルを全て開いて問題の正誤を記録した
- 成功したら即停止する
```
...
2. **単元**: 「どの単元のプリントを作りますか？（例: 回転体の表面積と体積）つまずきマーク付きの単元を自動検出する場合は『おまかせ』と言ってください」
...
## Phase 5: つまずき報告・単元選択（おまかせモードのみ）

**単元指定モードの場合はこの Phase をスキップする。**
