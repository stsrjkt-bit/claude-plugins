...
音声は fine-tuned Qwen3-TTS 1.7B（`YUKI66/qwen3-tts-1.7b-finetuned`、先生の声で学習済み）を Kaggle GPU で生成する。環境変数 `KAGGLE_API_TOKEN` と `HF_TOKEN` が必要。
...
# HF_TOKEN は source ~/.env.hf で設定済み
