...
音声は fine-tuned Qwen3-TTS 1.7B（`YUKI66/qwen3-tts-1.7b-finetuned`、先生の声で学習済み）を Modal serverless GPU で生成する。`modal` パッケージ + `modal token new` でセットアップ済みであること。
...
### Step 1.5: Modal で音声生成

Manim スクリプト内の全 `with self.voiceover(text="...")` のテキストを抽出し、Modal serverless GPU で一括生成する。
...
echo "SUPABASE_URL=${SUPABASE_URL} (token length: ${#ADMIN_ACCESS_TOKEN})"
```
- `~/.env.studygram` が存在しない場合は PDF をデスクトップに残してユーザーに報告する（動画・アップロードはスキップ）。
- トークンの内容は出力しない（長さのみ確認）。
...
Phase 11.5 で動画が生成されている場合、既存プリントに動画を紐付ける。
...
if [ -n "$VIDEO_PATH" ] && [ -f "$VIDEO_PATH" ]; then
  VIDEO_RESULT=$(curl -s -w "\n%{http_code}" "${SUPABASE_URL}/functions/v1/hoshu-video-upload" \
    ...
    -F "printId=${PRINT_ID}" \
    -F "video=@${VIDEO_PATH}")
  HTTP_CODE=$(echo "$VIDEO_RESULT" | tail -1)
  echo "Video upload HTTP: ${HTTP_CODE}"
fi
```

- `PRINT_ID` は Step 3 の PDF アップロードで取得済み
- 動画ファイルが存在しない場合（レンダリング失敗等）はスキップし、プリントのみアップロード
- HTTP ステータスが 200 以外の場合は動画アップロード失敗をユーザーに報告するが、プリント割り当て（Step 4）は続行する
