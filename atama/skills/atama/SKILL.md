---
name: atama
description: atama+ COACH 操作（学習データ取得→つまずき分析→補習プリント作成）
user_invocable: true
arguments:
  - name: target
...
## Phase 12: 出力分岐（中学 → デスクトップ / 高校 → StudyGram）

Phase 4 の高校生判定に基づいて出力先を分岐する。

### 中学生の場合

PDF は Phase 11 で `/mnt/c/Users/stsrj/Desktop/補習プリント/` に自動保存される（`generate-pdf.mjs` のデフォルト出力先）。

**完了報告:**
「`{単元名}_補習プリント.pdf` と `{単元名}_補習プリント_解答.pdf` をデスクトップの補習プリントフォルダに保存しました」

### 高校生の場合

PDF を StudyGram にアップロードし、対象生徒に割り当てる。アップロード後は一時ファイルを自動削除する。
...
```
- `~/.env.studygram` が存在しない場合は中学生と同じくデスクトップに保存する。
...
#### Step 7: 結果報告
- **成功時**: 「プリント『【補習】{単元名}』を {生徒名} のプリント棚に収納しました」
